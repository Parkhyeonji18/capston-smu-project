...
def showFemale(request):
    ...
    for loc in female_total:
        ...
        crime_location = {"type": "Feature",
                          "properties": {'area': math.ceil(round(contain_coordinate.length, 5) * 100000)},
...
def danger_map(request):
    ...
    for loc in danger_object_all:
        ...
        image_tag = '<body><div style="text-align:center;"><img src="data:image/jpeg;base64,{}" width="120"><div>'.format(
            pic)
...
def register_danger(request):
    ...
    if request.method == "POST":
        ...
        danger_string = str(point_danger_loc[0]) + " " + str(
            point_danger_loc[1])  # han : DB에 "37.566 126.9784"이런 식으로 들어가야함
